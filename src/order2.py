    ...
    def save_history_record(order_uuid, history_record, session):
        ...
        try:
            ...
            prepared_statement = session.prepare("""
                INSERT INTO order_history2 (
                    uuid, update_timestamp, market_price, stoploss, status,
                    evaluation_result, stopsize, user_id, is_matched, selling_price,
                    price_update_count,highest_price)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)
            """)
