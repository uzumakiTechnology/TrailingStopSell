...
kafka_producer_obj = KafkaProducer(
    ...
    batch_size=16384,  # Adjust based on your requirement
...
def send_price_update(producer, price, uuids):
    for uuid in uuids:
        message = {'price': price, 'uuid': uuid, 'timestamp': datetime.now().isoformat()}
        producer.send(KAFKA_TOPIC_NAME_CONS, value=message)
