...
def main():
    ...
    price_df = kafka_source_df.select(from_json(col("value").cast("string"), new_data_schema).alias("data")).select("data.*")

    def update_order_logic(order_dict, new_price):
        new_price = float(new_price if new_price is not None else 0.0)
        highest_price = new_price if new_price > float(order_dict.get('highest_price', 0)) else float(order_dict.get('highest_price', 0))


        is_matched_already = order_dict.get('is_matched', False)
        stoploss = float(order_dict.get('stoploss', 0))
        price_update_count = int(order_dict.get('price_update_count', 0)) + 1

        updated_highest_price = new_price if new_price > highest_price else highest_price
        updated_stoploss = new_price - float(order_dict.get('stopsize', 0)) if new_price - float(order_dict.get('stopsize', 0)) > stoploss else stoploss
